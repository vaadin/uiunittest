...
public class GridTester<T> extends Tester<Grid<T>> {
	...
	public Object cell(int column, int row) {
		assert (column > -1 && column < getComponent().getColumns().size()) : "Column out of bounds";
		assert (row > -1 && row < size()) : "Row out of bounds";
		T cat = (T) item(row);
		ValueProvider<T, ?> vp = getComponent().getColumns().get(column).getValueProvider();
	...
	public T item(int row) {
		assert (row > -1 && row < size()) : "Row out of bounds";
		return getComponent().getDataCommunicator().fetchItemsWithRange(row, 1).get(0);
	}
	...
	public void click(int column, int row) {
		...
		Grid.ItemClick<T> event = new Grid.ItemClick<T>(getComponent(), getComponent().getColumns().get(column), item,
				details, row);
		fireSimulatedEvent(event);
		if (getComponent().getSelectionModel() instanceof SingleSelectionModel) {
	...
	/**
	 * Simulate clicking of the multiselect checkbox column of the Grid as a user on
	 * a given item.
	 * 
	 * @param row The row index
	 */
	public void clickToSelect(int row) {
		assert (isInteractable()) : "Can't interact with disabled or invisible Grid";
		assert ((getComponent().getSelectionModel() instanceof MultiSelectionModel))
				: "Grid is not in multiselect mode";
		T item = item(row);
		clickToSelect(item);
	}
	...
	private void select(Set<T> items) {
		assert (getComponent().getSelectionModel() instanceof MultiSelectionModel) : "Grid is not multiselect";
		assert (items != null) : "Items can't be null";
		Set<T> copy = getComponent().getSelectedItems().stream().map(Objects::requireNonNull)
	...
	private void updateSelection(Set<T> copy, Set<T> removed) {
		MultiSelectionModelImpl<T> model = (MultiSelectionModelImpl<T>) getComponent().getSelectionModel();
		Class<?> clazz = model.getClass();
		try {
			Method updateSelectionMethod = clazz.getDeclaredMethod("updateSelection", Set.class, Set.class,
					Boolean.TYPE);
	...
	private void select(T item) {
		assert (getComponent().isEnabled()) : "Can't interact with disabled Grid";
		assert (getComponent().getSelectionModel() instanceof SingleSelectionModel) : "Grid is not singleselect";
		String key = getComponent().getDataCommunicator().getKeyMapper().key(item);
		SingleSelectionModelImpl<T> model = (SingleSelectionModelImpl<T>) getComponent().getSelectionModel();
		Class<?> clazz = model.getClass();
		try {
